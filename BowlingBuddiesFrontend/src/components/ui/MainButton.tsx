...
function MainButton({ buttonName, variant, classList }: ButtonProps) {
  if (variant === "fill") {
    return (
      <>
        <button
          className={`text-mh2 w-33.5 font-semibold bg-orange-100 py-2 px-8 rounded-m25 text-center cursor-pointer hover:bg-orangeHover-100 focus:border-2 focus:border-darkerBlue-100 active:w-30 active:py-1 ${classList}`}
        >
          {buttonName}
        </button>
      </>
